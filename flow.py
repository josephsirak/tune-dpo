...
class DPOPostTrainDemo(FlowSpec):
    ...
    # @model versions heavy directories, functionally the same as checkpoints.
    # Better used for passing a model between a training and inference step, 
    # it saves an object passable to metaflow.load_model, 
    # to be easily be loaded onto inference servers, notebook runtimes, etc.
    @model(
        load=[("llama_model", "metaflow-chkpt/model")], 
    ...
    def train(self):
        ...
        config = yaml.safe_load(self.training_config)
        config["base_dir"] = current.model.loaded["llama_model"]
    ...
    @card(type='html')
    @model(
        load=[("llama_model", "metaflow-chkpt/llama_model")], 
        temp_dir_root="metaflow-chkpt/loaded_models"
    )
    ...
    def eval(self):
        ...
        self.results = run_eval(
            checkpoint_path="metaflow-chkpt/llama_model",
